...
def open_file():
    ...
    if len(selectedBoxes.winfo_children())!=0 and annotationsSaved==False:
        MsgBox = tk.messagebox.askquestion('unsaved Annotations', 'you have unsaved annotations , do you want to save them ?', icon='warning')
        if MsgBox == 'yes':
            saveAnnotation()
    ...
    filePath = tkinter.filedialog.askopenfile(mode='rb', title="Select an image",
...
def createBoxElement(categorie,x_start,y_start,x_end,y_end):
    ...
    addBox = tk.Button(inputForm, text="add Box",bg="#676FA3",fg="white",
                       command=lambda: saveBoxData(categorie_input.get(), x_start_input.get(), y_start_input.get(),
                                                   x_end_input.get(), y_end_input.get()))
...
def saveBoxData(categorie, x1, y1, x2, y2):
    ...
    if verifOverlap(int(x1), int(y1), int(x2), int(y2)) :
        tkinter.messagebox.showerror("Overlap Error", " this box Overlap with other Boxes with more than 20% \n or the box surface is less than 40 pixels ")
    else :
...
def showSelectedBoxe(id,boxData):
    ...
    oneSelectedBox = tk.LabelFrame(selectedBoxes, text="box : "+str(id), pady=5, bg="white", borderwidth=1)
    oneSelectedBox.idTag=id
    info = tk.Label(oneSelectedBox, text=','.join(list(boxData.values())), bg="white")
    updateButton = tk.Button(oneSelectedBox, text="update", bg="#676FA3", fg="white",name=str(id),command=lambda: updateOneBox(id,oneSelectedBox))
...
def saveAnnotation():
    ...
    try:
        ...
        imageData={"urlImage": {"boxId": {"category": "", "x1": "", "y1": "", "x2": "", "y2": ""}}}
    ...
    annotationsSaved =True
    tkinter.messagebox.showinfo("Save Annotation",str(len(data))+" Annotations have been successfully saved in annotations.json")


def importCategories():
    global categoriesList
    # Opening JSON file
    pathFileCategories = tkinter.filedialog.askopenfile(mode='rb', title="Import Categories",
                                              filetypes=[("json file", "*.json")])
    f = open(pathFileCategories.name, )
    ...
    try:
        ...
        # Closing file
        f.close()
        tkinter.messagebox.showinfo("Import  Categories", str(nbCategries)+" Categories have been successfully imported")
    except:
        tkinter.messagebox.showerror("Error Import  Categories","Something went wrong ! please verify that your json file in this format \n {'categories':['category1','category1','category_n']}")
...
def addCategoryGui():
    ...
    addCategoryBtn = tk.Button(addCategoryForm, text="add",bg="#676FA3",fg="white",command=lambda : addCategoryFn(newCategoryInput.get(),addCategoryForm))
...
def addCategoryFn(newCategory,parent):
    ...
    if newCategory:
        ...
        #remove duplicate
        categoriesList = list(dict.fromkeys(categoriesList))
        tkinter.messagebox.showinfo("add  Category",
                                    newCategory + " have been successfully added")
...
def deleteCategoryGui():
    ...
    deletedCategoryLabel = tk.Label(deleteCategoryForm, text="category to be deleted :", anchor='w')
    deletedCategoryLabel.pack(fill=tk.X)
    categorie_input = ttk.Combobox(deleteCategoryForm, state="readonly", values=categoriesList)
    categorie_input.current(1)
    categorie_input.pack()

    #get the category to be replaced
    deleteCategoryBtn = tk.Button(deleteCategoryForm, text="delete", bg="#676FA3", fg="white",
                               command=lambda: replaceCategoryFn(" ",categorie_input.get(), deleteCategoryForm,"deleted"))
...
def replaceCategoryGui():
    ...
    replacedCategoryLabel = tk.Label(replaceCategoryForm, text="category to be replaced :",anchor='w')
    replacedCategoryLabel.pack(fill=tk.X)
    categorie_input = ttk.Combobox(replaceCategoryForm,state="readonly",values=categoriesList)
    categorie_input.current(1)
    categorie_input.pack()
    ...
    #get the category to be replaced
    replaceCategoryBtn = tk.Button(replaceCategoryForm, text="replace", bg="#676FA3", fg="white",
                               command=lambda: replaceCategoryFn(newCategoryInput.get(),categorie_input.get(), replaceCategoryForm,"replaced by"))
    ...
    if newCategory:
        ...
        for key, value in data.items():
            if value.get("category") == oldCategory:
                ...
                compteur+=1
        for child in  selectedBoxes.winfo_children():
                child.winfo_children()[0]['text'] =','.join(list(data.get(child.idTag).values()))


        tkinter.messagebox.showinfo("replace  Category",
                                    str(compteur) + " occurrences of "+oldCategory+" were successfully "+operation+  " "+newCategory)
...
def verifOverlap(XA1,YA1,XA2,YA2):
    ...
    for box in data.values():
        ...
        XB1,YB1,XB2,YB2 =int(box["x1"]), int(box["y1"]), int(box["x2"]), int(box["y2"])
...
middleFrame = tk.LabelFrame(app, text="middle", background='#EEF2FF', width=900, height=700)
rightFrame = tk.LabelFrame(app, text="right", background='#EEF2FF', width=300, height=700)

# lefFrame

uploadButon = tk.Button(leftFrame, text="upload Image", fg="white",bg="#676FA3", command=open_file)
uploadButon.pack(ipadx=200,ipady=10)
annotationSaving = tk.Button(leftFrame, text="save annotation",fg="white", bg="#676FA3",command=saveAnnotation)
annotationSaving.pack(ipadx=200,ipady=10)
importCategories = tk.Button(leftFrame, text="import categories",fg="white", bg="#676FA3",command=importCategories)
importCategories.pack(ipadx=200,ipady=10)
addCategory = tk.Button(leftFrame, text="add category", fg="white",bg="#676FA3" ,command=addCategoryGui)
addCategory.pack(ipadx=200,ipady=10)
deleteCategory = tk.Button(leftFrame, text="delete category", fg="white",bg="#676FA3" ,command=deleteCategoryGui)
deleteCategory.pack(ipadx=200,ipady=10)
replaceCategory = tk.Button(leftFrame, text="replace category", fg="white",bg="#676FA3",command=replaceCategoryGui)
replaceCategory.pack(ipadx=200,ipady=10)
help = tk.Button(leftFrame, text="help", fg="white",bg="#676FA3",command=lambda:webbrowser.open_new("help.pdf"))
help.pack(ipadx=200,ipady=10)

# middleFrame
imageArea = tk.Canvas(middleFrame, background="white", width=imageWidth, height=imageheight)
imageArea.pack()

# rightFrame
boxSelection = tk.Button(rightFrame, text="box selection",fg="white", bg="#676FA3", command=selectAbox,state=tk.DISABLED)

...
def open_file():
    ...
    if len(selectedBoxes.winfo_children()) != 0 and annotationsSaved == False:
        MsgBox = tk.messagebox.askquestion('unsaved Annotations',
                                           'you have unsaved annotations , do you want to save them ?', icon='warning')
        if MsgBox == 'yes':
            saveAnnotation()
...
def saveBoxData(categorie, x1, y1, x2, y2):
    ...
    if verifOverlap(int(x1), int(y1), int(x2), int(y2)):
        tkinter.messagebox.showerror("Overlap Error",
...
def saveAnnotation():
    ...
    if filePath:
        ...
        tkinter.messagebox.showinfo("Save Annotation",
                                    str(len(data)) + " Annotations have been successfully saved in annotations.json")
    else:
        tkinter.messagebox.showerror("Error Save annotation ",
...
def importCategories():
    ...
    try:
        ...
        # Closing file
        f.close()
        tkinter.messagebox.showinfo("Import  Categories",
                                    str(nbCategries) + " Categories have been successfully imported")
    except:
        tkinter.messagebox.showerror("Error Import  Categories",
...
def addCategoryGui():
    ...
    addCategoryBtn = tk.Button(addCategoryForm, text="add", bg="#676FA3", fg="white",
...
def deleteCategoryGui():
    ...
    deleteCategoryBtn = tk.Button(deleteCategoryForm, text="delete", bg="#676FA3", fg="white",
                                  command=lambda: replaceCategoryFn(" ", categorie_input.get(), deleteCategoryForm,
                                                                    "deleted"))
...
def replaceCategoryGui():
    ...
    replaceCategoryBtn = tk.Button(replaceCategoryForm, text="replace", bg="#676FA3", fg="white",
                                   command=lambda: replaceCategoryFn(newCategoryInput.get(), categorie_input.get(),
                                                                     replaceCategoryForm, "replaced by"))
...
help = tk.Button(leftFrame, text="help", fg="white", bg="#676FA3", command=lambda: webbrowser.open_new("Documentation.pdf"))
...
# rightFrame
boxSelection = tk.Button(rightFrame, text="box selection", fg="white", bg="#676FA3", command=selectAbox,
                         state=tk.DISABLED)

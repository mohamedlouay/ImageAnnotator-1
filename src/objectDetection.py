...
train_generator = img_dataGen.flow_from_directory(directory=r'D:\M1\NNL\Projet\ImageAnnotator\img', target_size=(100, 100),
                                                  color_mode='rgb', seed=42, class_mode='binary', subset='training', batch_size=8)

valid_generator = img_dataGen.flow_from_directory(directory=r'D:\M1\NNL\Projet\ImageAnnotator\img', target_size=(100, 100),
...
cnnModel = Sequential(
    [
        Conv2D(
            filters=32, kernel_size=(3, 3), activation="relu", input_shape=(100, 100, 3), padding='same'
        ),
...
history = cnnModel.fit(train_generator, epochs=30,
...
for i in range(2, 10):
    img_directory = 'D:/M1/NNL/Projet/ImageAnnotator/inputImages/imagemask ('+str(
        i)+')' + '.png'
    img_pred = image.load_img(img_directory, target_size=(100, 100))
    img_pred = image.img_to_array(img_pred)
    img_pred = np.expand_dims(img_pred, axis=0)

    # load the input image from disk, resize it such that it has the
    # has the supplied width, and then grab its dimensions
    orig = cv2.imread(img_directory)
    ...
    start = time.time()
    predictions = cnnModel.predict(rois)
    end = time.time()
    ...
    print(predictions)
    preds = imagenet_utils.decode_predictions(preds=predictions, top=1)
    labels = {}
